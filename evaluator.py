...
from pyt_utils import load_model, link_file, ensure_dir
...
class Evaluator(object):
    ...
    def mid_run(self, model_mid, log_file, log_file_link):
        results = open(log_file, "a")
        link_file(log_file, log_file_link)

        self.val_func = model_mid
        if len(self.devices) == 1:
            result_line = self.single_process_evalutation()
        else:
            result_line = self.multi_process_evaluation()
    ...
    def whole_eval(self, img, output_size, device=None):
        ...
        for s in self.multi_scales:
            ...
            processed_pred += cv2.resize(
                pred.cpu().numpy(), (output_size[1], output_size[0]), interpolation=cv2.INTER_LINEAR
            )
    ...
    def sliding_eval_rgbX(self, img, modal_x, crop_size, stride_rate, device=None):
        ...
        for s in self.multi_scales:
            ...
            processed_pred += self.scale_process_rgbX(
                img_scale, modal_x_scale, (ori_rows, ori_cols), crop_size, stride_rate, device
            )
    ...
    def process_image_rgbX(self, img, modal_x, crop_size=None):
        ...
        p_img = normalize(p_img, self.norm_mean, self.norm_std)
        # print(p_modal_x.shape)
        # p_modal_x = normalize(p_modal_x, [0.48,0.48,0.48],[0.28,0.28,0.28])
        if len(modal_x.shape) == 2:
            pass
        #    p_modal_x = normalize(p_modal_x, 0, 1)
            ...
            # print('mean norm---------------------------')
            # pass
            p_modal_x = normalize(p_modal_x, [0.48, 0.48, 0.48], [0.28, 0.28, 0.28])  # self.norm_mean, self.norm_std)
