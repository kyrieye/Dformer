...
class RGBXDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self._split_name == "train":
            img_path = self._train_source+"/"+item_name
        elif self._split_name == "val":
            img_path = self._eval_source+"/"+item_name

        class_name = item_name.split("_")[0]
        label_index = self.class_names.index(class_name)

        rgb_mode = "RGB"  

        img = self._open_image(img_path, rgb_mode)
        ...
        output_dict = dict(data=img, label=label_index)
